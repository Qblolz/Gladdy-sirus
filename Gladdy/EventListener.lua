...
function EventListener:detectConstellation(unit)
	...
	for key = 1, 40 do
		local arg1, icon, _, _, _, duration, expirationTime, _, _, spellID = UnitAura(unit, key, "HARMFUL")
...
function EventListener:COMBAT_LOG_EVENT_UNFILTERED(...)
	local _,eventType,_,sourceGUID,sourceName,sourceFlags,_,destGUID,destName,destFlags,_,spellID,spellName,spellSchool,extraSpellId,extraSpellName,extraSpellSchool = CombatLogGetCurrentEventInfo(...)
